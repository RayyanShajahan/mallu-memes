...
with tabs[1]:
    ...
    if capture_mode == "Continuous Live Stream (WebRTC)":
        if WEBRTC_AVAILABLE:
            ...
            st.info("WEBRTC ENGINE: STUN Server Active (stun.l.google.com:19302)")
            ...
            class EmotionProcessor(VideoProcessorBase):
                ...
                def recv(self, frame):
                    ...
                    try:
                        ...
                        pass
                    cv2.putText(img, f"PSYCHE: {self.emotion.upper()}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                    return av.VideoFrame.from_ndarray(img, format="bgr24")

            ctx = webrtc_streamer(key="scanner", video_processor_factory=EmotionProcessor, rtc_configuration=RTC_CONFIGURATION)
            if ctx.video_processor:
                detected_emotion = ctx.video_processor.emotion
        else:
            st.warning("streamlit-webrtc not available in environment. Switching to Simulator mode.")
            capture_mode = "Emotion Simulator (Test Matrix)"

    elif capture_mode == "Instant Snapshot Frame (Camera Input)":
        cam_image = st.camera_input("Capture facial expression")
        ...
        detected_emotion = st.selectbox(
            "Simulate Facial Micro-Expression:",
            ["sad", "angry", "happy", "neutral", "fear", "surprise"]
        )

    # Emotion Routing Matrix mapping
    emotion_map = {
        "sad": "KTU Exam Trauma",
    ...
    st.success(f"🎯 Detected Psyche: **{detected_emotion.upper()}** ➔ Routing to Category: **{target_category}**")

    # Filter Parquet Database for matching meme
    matched_df = df[df["emotion"] == detected_emotion]
    if matched_df.empty:
        matched_df = df # Fallback to full set

    top_meme = matched_df.sort_values(by="kerala_existential_weight", ascending=False).iloc[0]

    # Render Meme Card + Actual Image
    col_img, col_txt = st.columns([1, 1])
    
    with col_img:
        st.markdown("### 🖼️ Matched Meme Artifact")
        # Check if local asset image exists, else show dynamic visual card
        ...
        if asset_files:
            ...
            # Fallback placeholder visual banner if asset folder is empty
            st.error("Missing local meme assets. Run `create_sample_assets.py` to generate meme JPEGs.")
            st.markdown(f"**[{top_meme['character']} - {top_meme['movie']}]**")

    with col_txt:
        st.markdown(f"### 🎭 {top_meme['character']} — {top_meme['movie']}")
        st.markdown(f"**Scenario:** {top_meme['scenario_title']} | **Archetype:** {top_meme['character_archetype']} | **KEW Score:** {top_meme['kerala_existential_weight']}/10")
